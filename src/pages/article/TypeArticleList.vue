<template xmlns:c="http://www.w3.org/1999/XSL/Transform">
  <div class="box container" style="margin-top: 15px">
    ...
    <div class="place col-sm-7 col-xs-12" id="pageContents" style="background-color:rgba(255,255,255,0)">
      ...
      <p style="color: #8c8c8c; font-size: large">分类 : {{ articleType.detail }}</p>
        ...
        <nav aria-label="Page navigation">
          <ul class="pagination pagination-lg">
            ...
            <li v-if="pageInfo.hasPreviousPage">
              <router-link :to="{ name:'typeArticle', query:{ type:type, page:pageInfo.prePage, searchValue: searchValue } }">
            ...
            <template v-for="pa in pageIter">
              <template v-if="pa === pageInfo.pageNum">
                <li class="active">
                  <router-link :to="{ name:'typeArticle', query:{ type:type, page:pa, searchValue:searchValue } }">
                    <a>
                      {{ pa }}
            ...
            <li v-if="pageInfo.hasNextPage">
              <router-link :to="{ name:'typeArticle', query:{ type:type, page:pageInfo.nextPage, searchValue:searchValue } }">
                <a aria-label="Next">
    ...
    export default {
            ...
            searchValue:{
                handler(newVal, objVal) {
                    if (newVal.length < 1 ) {
                        ...
                        this.$router.push( {
                            name:"typeArticle",
                            query:{ type:this.$route.query.type, page:"1", searchValue: " " }
                        } )
                    }
                }
            },
        ...
        methods: {
            ...
            skipArticleList() {
                ...
                this.$router.push( {
                    name:"typeArticle",
                    query:{ type: this.$route.query.type, page:"1", searchValue: this.searchValue }
                } )
            },
            // 搜索图标点击事件
            searchIcoClick() {
            ...
            loadData() {
                ...
                // 发起请求
                this.$http.get("/article/typeArticles/" + type + "/" + page + searchValue
                ).then(resp => {

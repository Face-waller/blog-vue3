<template>
  <div class="box container" style="margin-top: 15px">
    ...
    <div class="place col-sm-7 col-xs-12" id="pageContents" style="background-color:rgba(255,255,255,0)">
      ...
      <p style="color: #8c8c8c; font-size: large">分类 : 我的文章</p>
    ...
    <div class="blogs">
      ...
      <div class="bloglist" style="background-color:rgba(255,255,255,0)">
        <template v-for="a in pageInfo.list">
          ...
          <p>{{ a.descriptive }}         <a style="color: #00a2d4;" :href="'http://api.blogll.cn/api/article/articleModify/' + a.id">
              [编辑]
            </a>       <a style="color: #b92c28;cursor:pointer" @click="deleteVerify(a.id)">
              [删除]
            </a>
          </p>
        </template>
        ...
        <nav aria-label="Page navigation">
          <ul class="pagination pagination-lg">
            ...
            <li v-if="pageInfo.hasPreviousPage">
              <router-link :to="{ name:'myArticle', query:{ page:pageInfo.prePage, searchValue: searchValue } }">
            ...
            <template v-for="pa in pageIter">
              <template v-if="pa === pageInfo.pageNum">
                <li class="active">
                  <router-link :to="{ name:'myArticle', query:{ page: pa, searchValue: searchValue } }">
                    <a>
                        {{ pa }}
            ...
            <li v-if="pageInfo.hasNextPage">
              <router-link :to="{ name:'myArticle', query:{ page:pageInfo.nextPage, searchValue:searchValue } }">
                <a aria-label="Next">
    ...
    export default {
        ...
        methods: {
            ...
            deleteVerify(id) {
                this.$confirm('此操作将删除该文章, 是否继续?', '提示', {
                    ...
                    if (id < 0 || id === undefined || id == "") {
                        this.open("提示","删除的文章id不正确");
                        return;
                    }
                    this.$http.delete("/article/articleDeleteMarkTrue/" + id
                    ).then(resp => {
            ...
            loadData() {
                ...
                // 发起请求，获得数据
                this.$http.get("/article/myArticles/" + page + searchValue
                ).then(resp => {
                    // console.log(resp.data); // 返回的Response体
                    // console.log(resp.data.username);
